...
print("\n[1回目が「冷えた回」であることが、両方に残っているか]")
# **結果を見る前に決めた約束。**あとから消せてしまうと、
# 都合の悪い偏りだけ落ちる。両方の文書で見張る
ck("手順書: 1回目が必ず G に当たると書いてある", "1回目は必ず G" in tej)
ck("手順書: 偏りの向き（G に不利）が書いてある", "G に不利" in tej)
ck("手順書: 断定しないと書いてある", "断定" in tej)
ck("手順書: 突出の判定基準がある",
   "|G2 − G3|" in tej and "両方より大きい" in tej)
ck("手順書: 足さなかったときも書くと約束している",
   "足さなかった理由" in tej)
ck("手順書: やり直したら判定を使えないと書いてある",
   "やり直した場合" in tej)
ck("チェックリスト: 1回目に注記がある", "冷えた回" in chk)
ck("チェックリスト: 空回しを禁じている", "空回し" in chk)
ck("チェックリスト: 毎回起動し直すと書いてある", "毎回、起動し直します" in chk)

print("\n[6回目の前に登録した予測が残っているか]")
# **外れた予測を消せないようにする。**残っていること自体を見張る
ck("手順書: 予測の節がある", "6回目の前に登録した予測" in tej)
ck("手順書: 2 つの説明を分ける表がある",
   "出力先で決まる" in tej and "空き時間で決まる" in tej)
ck("手順書: どちらに出ても書くと約束している", "外れたほうを落とさない" in tej)
