...
#
# **2026-08-22 に利用者が音声を聴いて確かめた。**01:05:48 の
# 「失礼いたします。」3 区間は、**同じ音声を 3 回書いた暴走**だった
# (実際の音声は「失礼いたします。これで失礼いたします」の 2 発言だけ)。
# 中央値 18.2 字/秒。本物の最大は 13.9 字/秒なので、**15 はこの 2 つの
# 間に正しく入っている。**
#
# 一度 25 へ上げかけたが、「3 回とも本物」という誤解にもとづいていた。
# 聴いた人の判定が入って初めて分かったので、**耳で確かめた事実だけを
# 根拠にすること。**
...
class LocalTranscriber:
                ...
                if on_log:
                    ...
                    if self.model != was:
                        on_log(
                            ...
                            "固有名詞の誤りが増えます"
                            "(実測: 利用者が誤りとして挙げた 5 語のうち、"
                            f"{was} は 5 語とも拾い、{self.model} は 2 語)。")
                        on_log(
                            "  ※ GPU を使うには、音声を選び直したときに出る"
                            "案内から部品を取得してください。"
                            f"{was} で起こし直すと固有名詞の誤りが減ります。")
                    else:
                        on_log("  CPU に切り替えます。")
                try:
                    self._whisper = build(self.device, self.compute_type)
    ...
    def transcribe(
        ...
        *,
        ...
        #
        # **他の 2 分岐と同じ数字を出す。**ここだけ字数を出しておらず、
        # **どちらの結果を残したのかも言っていなかった**（2026-08-31 に
        # 実測で気づいた）。字数こそが「本物の相づちか、本文が消えたか」を
        # 分ける材料なのに、それが読み手に渡っていなかった。
        #
        # **断定しない。**引き継ぎを切っても同じ繰り返しが出たということは、
        # 独立した 2 回の復号が同じものを見たということでもある。実データでは
        # 本物の相づち（会議終わりの挨拶）だった例がある——同じ箇所が 6 回とも
        # 出て、字数もいちばん少ない回ではなかった。**聴いてもらうしかない。**
        if on_log:
            on_log(f"  ※ 起こし直しても直りませんでした"
                   f"({n} 区間 → {n2} 区間の繰り返し / "
                   f"本文 {before:,} 字 → {after:,} 字)。元のままにします。"
                   "引き継ぎを切っても同じ繰り返しが出たので本物の相づちかも"
                   "しれませんが、本文が失われている可能性も残ります。"
                   "この付近を聴いて確かめてください。")
        return result
