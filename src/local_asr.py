...
#
# **15 では誤発動した。**2026-08-22 に利用者が音声を聴いて確かめたところ、
# 01:05:48 の「失礼いたします。」3 回は**本物**(3 人が続けて挨拶していた)。
# ここで気づいたのは、**同じ音声・同じモデルでも走らせるたびに字/秒が動く**
# こと:
#
#   時刻       本文              判定    前回    今回
#   01:05:48   失礼いたします。   本物   13.3   18.2
#   00:12:33   はい。            暴走   12.7   20.0
#
# 5 字/秒 も動くので、15 という線は測定の揺れの中に埋まっていた。しかも
# 実データ 8 件で見ると、**字/秒の線が単独で捕まえている暴走は 1 件も無い**
# (暴走 3 件はすべて回数 6 回以上でも捕まる)。本物の最大が 18.2 だったので、
# 37% の余裕を取って 25 にする。44 字/秒のような**人間に不可能な速度**は
# 残したいので、線を外すのではなく上げる。
#
# 効果: この日の実会議で、無駄な起こし直しが 3 回 → 0 回。転写 21 分 → 15 分。
