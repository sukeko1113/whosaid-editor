...
#
# 【英語テスト用ブランチ】日本語版は 8。英語では話速の比(11.0 / 4.5 ≒ 2.4)
# ぶん引き上げないと同じ「一致の重み」にならない。加えて 8 は MIN_BLOCK = 10 を
# 下回っており、そのままでは一度も発火しない死んだ閾値になる。
MIN_MATCHED = 22
...
#
# 【英語テスト用ブランチ】日本語版は 3。英語の 3 文字は 1 語に満たず、
# 語尾の "-ing" が欠けただけで終了を捨ててしまう。1 語ぶん(平均 5 文字)を
# 超えて欠けたときに疑う、という意味で 8 にした。
TAIL_GAP_LIMIT = 8
...
#
# 【英語テスト用ブランチ】日本語版は 1.5(正常な発話が実測 4〜11 字/秒)。
# 英語は空白を落とした状態で約 11 字/秒あり、1.5 では選別として働かない。
# 日本語で「正常の下端の約 1/3」だった比を保って 4.0 にした。
MIN_DENSITY = 4.0
