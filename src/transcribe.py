...
# 英語の話速の目安。150 wpm = 2.5 語/秒、1 語平均 5 文字(空白込み)で
# 約 12.5 文字/秒。ここは len() を生の本文(空白込み)に取るので空白を
# 数えてよい。
#
# 値を 12.5 ではなく 11.0(約 132 wpm)にしてあるのは、この定数が
# 意図的に低めへ倒してあるため。日本語版は実測 4〜6 文字/秒に対して 4.5、
# つまり範囲の下端を採っていた。速度を低く見積もるほど時間を長く確保し、
# 避けたい「発言が途中で切れる」側に倒れない。加えて日本の高校英語
# ディベートは原稿を読む形式が主で、150 wpm より遅い 120〜140 wpm が実情。
#
# 【英語テスト用ブランチ】日本語版は 4.5。
SPEECH_CHARS_PER_SECOND = 11.0
