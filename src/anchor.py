...
# 一致とみなす最短の長さ。
#
# 【英語テスト用ブランチ】日本語版は 3(日本語は 1〜2 文字の一致が偶然でも
# 頻発するため)。英語は 3 文字では the / and / for が当たり続けるので上げた。
#
# 値は実測で決めた。照合器が見るのは normalize() 後の**空白を落とした**
# 文字列(both sides)なので、英語も 1 本の文字列として衝突する。
# 無関係なディベート英語どうしを突き合わせたときの最長偶然一致:
#
#     区間 40 字 × 窓 375 字(30 秒)  中央値 5 / p95  9
#     区間 100 字 × 窓 375 字         中央値 8 / p95 10
#     区間 200 字 × 窓 375 字         中央値 8 / p95 17
#
# 同じ試験を無関係な日本語で行うと最長 2 文字だった。
# 10 は現実的な長さでの p95 を上回り、英語 2 語ぶんに当たる。日本語で 3 が
# 担っていた重みとほぼ同じ。
#
# これが効くのは measure() が blocks[0] と blocks[-1] から開始・終了を取る
# ため。窓の端に偶然の 9 文字が 1 つあるだけで区間の境界がそこへ動く。
#
# 短い応答("Yes" = 3 字)はこの閾値を下回り、measure() が None を返して
# 「照合不能」になる。これは意図した挙動で、本文には一切触れない
# (日本語の「はい」= 2 字も MIN_BLOCK = 3 の下で同じだった)。
# 短い区間だけ閾値を下げる仕掛けは**入れない**。入れると "Yes" が 30 秒の
# 窓の中のどの yes にも当たり、自信を持って間違った時刻を出す。
MIN_BLOCK = 10
