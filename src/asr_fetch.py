...
# ======================================================================
# **SHA-256 をここでは固定しない。**話者分離側と揃えていない理由（2026-08-31）
#
# `tools/fetch_diarize_models.py` は 2 つのモデルの SHA-256 を固定値で持ち、
# 取得の前後で照合する。**こちらは持たない。片手落ちに見えるが、意図的である。**
#
# 取得元の性質が違う:
#
#   話者分離  GitHub Releases の生 URL
#             → **配布側にハッシュ照合の仕組みが無い。**落としたものが
#                意図したものかを確かめる手段が、こちらで持つ固定値しかない
#
#   ASR       Hugging Face Hub（`huggingface_hub.snapshot_download`）
#             → **LFS プロトコル自体がファイルごとのハッシュを持ち、
#                ダウンロード時に照合する。**こちらで二重に持つ必要が薄い
#
# つまり「検証していない」のではなく、**検証の主体が違う。**
# 片方だけ固定値を書いているのは、片方にしか仕組みが無いためである。
#
# **固定値を持つ代償**も考えたうえで採らなかった: モデルが更新されるたびに
# こちらの定数を直す必要があり、直し忘れると**正しいモデルを弾く**。
# 話者分離側は版が固定された配布物なのでこの代償が小さいが、ASR は
# 4 モデル × 更新のたびになる。
#
# **見直す条件**: `snapshot_download` をやめて生の URL から取るようになったら、
# そのときは固定値が要る（配布側の照合が無くなるため）。
# ======================================================================

# **これが無いと動かない**という核。取得後に揃っているかを確かめる。
# ハッシュは上の理由で見ないが、**揃っているかは見る**——途中で切れた取得を
# 「成功」と扱わないため。
